...
// Problem 1 - All last names need to be listed in alphabetical order.
let lastInOrder = runners.sort((a, b) => a.last_name.localeCompare(b.last_name));
//console.log(lastInOrder);

// Problem 2 - Only the donors of at least $200.00 or more are allowed to the cocktail party after the 5k run. Please prepare a list of those runners.
let bigSpenders = runners.filter(runner => runner.donation >= 200);
//console.log(bigSpenders);
// Problem 3 - One of the busses to pick up the runners is running late due to mechanical issues.  Prepare a list for the second half of the runners (26-50) that will ride on the new bus.
let newBusRiders = runners.filter(rider => {
    return rider.id> 25;
});
console.log(newBusRiders);